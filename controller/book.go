...
func New(s entities.Service) entities.Handler {
...
func (bc *BookController) InsertBook() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		_, err := bc.service.InsertBook(entities.Core{
			Title:    input.Title,
			Year:     input.Year,
			Author:   input.Author,
...
func (bc *BookController) GetBookByBookID() echo.HandlerFunc {
	return func(c echo.Context) error {
		inputID := c.Param("id")
		if inputID == "" {
			return c.JSON(http.StatusBadRequest, map[string]interface{}{
		...
		book, err := bc.service.GetBookByBookID(uint(bookID))
		if err != nil {
			c.Logger().Error("terjadi kesalahan", err.Error())
			return c.JSON(http.StatusInternalServerError, map[string]interface{}{
				"code":    http.StatusInternalServerError,
...
func (bc *BookController) UpdateByBookID() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := bc.service.UpdateByBookID(uint(bookID), update.ToEntity()); err != nil {
...
func (bc *BookController) DeleteByBookID() echo.HandlerFunc {
	return func(c echo.Context) error {
		bookID, err := strconv.ParseUint(c.Param("id"), 10, 32)
		if err != nil {
			c.Logger().Error("Failed to parse book ID", err.Error())
		...
		if err := bc.service.DeleteByBookID(uint(bookID)); err != nil {
